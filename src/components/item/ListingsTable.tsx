  ...
  return (
    <div>
      <div className="mb-3 flex items-center gap-3">
        <label className="flex cursor-pointer items-center gap-2 text-sm text-obsidian-300">
          <input
            type="checkbox"
            checked={hqOnly}
            onChange={(e) => setHqOnly(e.target.checked)}
            className="rounded border-obsidian-700 bg-obsidian-800 text-gold-500 focus:ring-gold-500"
          />
          僅顯示 HQ
        </label>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-obsidian-800">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-obsidian-800">
            {listings.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-obsidian-500">
                  無上架資料
                </td>
              </tr>
            ) : (
              listings.map((l) => (
                ...
                >
                  <td className="px-4 py-3 text-obsidian-200">{l.world_name}</td>
                  <td className="px-4 py-3">
                    <GilAmount amount={l.price_per_unit} />
                  </td>
                  <td className="px-4 py-3 font-mono text-obsidian-300">{l.quantity}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-obsidian-400">{l.retainer_name}</td>
                </tr>
              ))
            )}
          </tbody>
