    ...
    <PageShell title="撿漏排行">
      <div className="flex flex-wrap gap-4">
        ...
        <label className="flex flex-col gap-1 text-sm text-zinc-400">
          伺服器
          <select
            className="w-40 rounded-md border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-zinc-100 focus:border-gold-500 focus:outline-none"
            value={world ?? ""}
          ...
          <tbody>
            ...
            {items.map((item) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-gold-400">{formatGil(item.current_price)}</td>
                <td className="px-4 py-3 text-zinc-300">{formatGil(item.average_price)}</td>
                <td className="px-4 py-3 text-emerald-400">{item.discount.toFixed(1)}%</td>
              </tr>
            ))}
          </tbody>
        </table>
